...
const char *WINDOWTITLE = { "OpenGL / Topic Geometric Modeling" };
...
void
Givepoint(Point *P0, Point *P1, Point *P2, Point *P3) 
{
    ...
    for (float i = 0.; i <= 1.; i = i + 0.01) {
        ...
        x0 = x0 + ((i * i) * ((2. * P0->x) - (5. * P1->x) + (4 * P2->x) - P3->x));
        y0 = y0 + ((i * i) * ((2. * P0->y) - (5. * P1->y) + (4 * P2->y) - P3->y));
        z0 = z0 + ((i * i) * ((2. * P0->z) - (5. * P1->z) + (4 * P2->z) - P3->z));

        x0 = x0 + ((i * i * i) * ((-P0->x) + (3 * P1->x) - (3 * P2->x) + P3->x));
        y0 = y0 + ((i * i * i) * ((-P0->y) + (3 * P1->y) - (3 * P2->y) + P3->y));
        z0 = z0 + ((i * i * i) * ((-P0->z) + (3 * P1->z) - (3 * P2->z) + P3->z));
...
Show_Point(Point * p) {
	...
	glPushMatrix();
		...
		gluSphere(quad, 0.08, 50, 50);
	glPopMatrix();
}

void
Call_Matchstick_Men(Point *P0, Point *P1, Point *P2, Point *P3, Point * P4, Point * P5) {
	if (S_Point) {
		Show_Point(P0);
		Show_Point(P1);
		Show_Point(P2);
	...
	if (S_Line) {
		Givepoint(P4, P0, P1, P2);
		Givepoint(P0, P1, P2, P3);
		Givepoint(P1, P2, P3, P5);
	}
	else {
		glBegin(GL_LINE_STRIP);
...
Display( )
{
	...
	//if (Vert) {
		if (t)
			Time1 = Time1 + sin(0.001);
		else
			Time1 = Time1 - sin(0.001);
	//}

	//glCallList( BoxList );
	struct Point *p0 = new struct Point;
    p0->x = -1.5;
    p0->y = Time1;
    p0->z = 0.;
    ...
    p3->y = -Time1;
    p3->z = 0.;
	struct Point *p4 = new struct Point;
	p4->x = p0->x - 3.;
	p4->y = p0->y;
	p4->z = p0->z;
	struct Point *p5 = new struct Point;
	p5->x = p3->x + 3.;
	p5->y = p3->y;
	p5->z = p3->z;
    ...
    glPushMatrix();
		...
		glColor3f(1., 0., 0.);
		Call_Matchstick_Men(p0, p1, p2, p3, p4, p5);
    glPopMatrix();
