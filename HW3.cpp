...
const char *WINDOWTITLE = { "OpenGL / Topic : Texture Mapping" };
...
InitMenus( )
{
	...
	int showimgmenu = glutCreateMenu(DoShowimgMenu);
	glutAddMenuEntry("Off", 0);
	glutAddMenuEntry("On", 1);
	...
	glutAddSubMenu(   "Texture",       showimgmenu);
...
InitGraphics( )
{
    ...
    glTexEnvf( GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE );
    glTexImage2D( GL_TEXTURE_2D, 0, 3, 512, 512, 0, GL_RGB,GL_UNSIGNED_BYTE, TextureArray0 );
