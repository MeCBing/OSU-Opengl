...
const char *WINDOWTITLE = { "OpenGL / Topic Shaders" };
...
MjbSphere( float radius, int slices, int stacks, float time)
{
	...
	for( int ilat = 0; ilat < NumLats; ilat++ )
	{
		...
		for( int ilng = 0; ilng < NumLngs; ilng++ )
		{
			...
			if( Distort )
			{
				p->s = ( lng + M_PI    ) / ( 2.*M_PI ) + sin((lat + M_PI/2. + (100*Time))) * 0.05;
				p->t = (lat + M_PI / 2.) / M_PI;
				//+sin(25.0 * p->y + 30.0 * p->x + 6.28 * 0.3477) * 0.05;
				//printf("Distort : s= %f t= %f\n", p->s, p->t);
			}
			else
			{
				p->s = ( lng + M_PI    ) / ( 2.*M_PI );
				p->t = ( lat + M_PI/2. ) / M_PI;
				//printf("Not Distort : s= %f t= %f\n", p->s, p->t);
			}
		}
	}
...
Display( )
{
    ...
    //SetSpotLight(GL_LIGHT2, 0., 0., 0., 5., 4., 0., 0., 0., 0.);
	S0 = 0.2;
	T0 = 0.2;
	Ds = 0.2;
	Dt = 0.2;
	...
	glPushMatrix();
		...
		//SetMaterial(1., 1., 0., 5.);
		Pattern->Use( );
		Pattern->SetUniformVariable( "uTime",  vT );
		...
		Pattern->SetUniformVariable( "uRand", Rand );

		Pattern->SetUniformVariable( "uColor", 1., 1., 0. );
		Pattern->SetUniformVariable( "uSpecularColor", 1., 1., 1. );
		//uS0 - uSize/2. <= vST.s && vST.s <= uS0 + uSize/2.
		
		//printf(gl_MultiTexCoord0.st);
		MjbSphere(1.3, 100, 100, Time);
		S0 = 0.3 + sin(fT);
		T0 = 0.5 + sin(fT);
		Size = 0.1 + fT;
		...
		glRotatef(90., 1., 0., 0.);
		gluCylinder(quadratic, 0.1, 0.1, 5., 50, 50);
