...
const char *WINDOWTITLE = { "OpenGL / Topic Lighting" };
...
Display( )
{
	...
	//glEnable(GL_LIGHT0);
	//SetSpotLight(GL_LIGHT1, 0., 0., 0., 0., 0., -5., 1., 1., 1.);
	SetPointLight(GL_LIGHT1, 4., 4., 0., 1., 1., 1.);
	if( Light_1 )
		glEnable( GL_LIGHT1 );
	else
		glDisable( GL_LIGHT1 );
	...
	glPushMatrix();
		...
		if(point <= -6.)
			a = true;
		if (a)
			point = point + sin(0.003);
		else
			point = point - sin(0.003);

		glShadeModel( GL_FLAT );
		SetMaterial(1., 0., 0., 128.);
		glTranslatef(0., 0., point);
		glCallList(BoxList);
	...
	glPushMatrix();
		glShadeModel( GL_SMOOTH );
		SetMaterial(0., 1., 0., 20.);
		glTranslatef(0., 0., -3.);
		glCallList(Torus);
		SetMaterial(1., 0., 0., 5.);
		glTranslatef(-3., 3., 1.62);
		SetSpotLight(GL_LIGHT2, 0., 0., 0., 5., 4., 0., 1., 0., 0.);
		if( Light_2 )
		{
			glDisable( GL_LIGHTING );
			glEnable( GL_LIGHT2 );
