...
const char *WINDOWTITLE = { "OpenGL / Animate a Helicopter" };
...
bool    IsInside;               // view inside or outside
bool	Frozen;                 // stop the animation 
float   Time;
float   CurrentTime;
int 	LookAT;
float 	Topbladeangle;
float 	Rearbladeangle;
...
Display( )
{
	...
	Topbladeangle = 360 * Time;
	Rearbladeangle = 360 * 3 * Time;

	glPushMatrix();
	glTranslatef( 0., 0., -15.);
	glRotatef(Topbladeangle, 0., 0., 1.);
	glCallList(Torus);
	glPopMatrix();
	
	//printf("%f          %f\n", Topbladeangle, Rearbladeangle);
	glPushMatrix();
	...
	glRotatef(Rearbladeangle, 1., 0., 0.);
    glCallList(Rearblade);
	glPopMatrix();
	...
	glLoadIdentity( );
	glColor3f( 1., 1., 1. );
	DoRasterString( 5., 5., 0., "My Helicopter" );
...
InitMenus( )
{
	...
	int lookatmenu = glutCreateMenu(DoLookAt);
...
InitLists( )
{
    ...
    Rearblade = glGenLists( 1 );
	glNewList( Rearblade, GL_COMPILE );
        glPushMatrix( );
        //glTranslatef(.5, 2.5, 9.);
        glRotatef(90., 0., 1., 0.);
		//glRotatef(Rearbladeangle, 1., 0., 0.);
